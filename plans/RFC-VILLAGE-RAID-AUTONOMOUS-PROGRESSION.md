...
| **Status** | `DESIGNING` → **V2 LOCK-CLEAN — ready for task-47** |
...
| **Last update** | 2026-08-15 (User peer review pass 2 — D-VR-071/066/067 + V2-E/F sequencing repair) |
...
| **V2-E** *(CLOSED 2026-08-16 — STATIC ACCEPT)* | `TradeWithVillagerGoal` @ priority **3** with one selected `TradeDemandGate` owner; introduces `ActivityClass.VILLAGE_TRADE` enum value and goal class (executor may stub until adapter wired) |
| **V2-F** *(CLOSED 2026-08-16)* | `MoveHolderClassifier` pin: `TradeWithVillagerGoal` → `VILLAGE_TRADE` — **co-lands with V2-E or immediately follows in the same task batch** (`D-VR-073`); classifier cannot precede the goal type it classifies |
...
V2-A (adapter) → V2-B (evaluation) → V2-C (demand registrar) → V2-D (chain ticket)
        ↓
V2-E (goal + TradeDemandGate + VILLAGE_TRADE enum/goal class)
        ↓
V2-F (MoveHolderClassifier pin) — same task batch as V2-E or next commit; never before E (D-VR-073)
        ↓
