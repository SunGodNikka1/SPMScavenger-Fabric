# VR-T3k — multi-mob crop contention.
...
setblock ~6 ~-1 ~4 minecraft:farmland
setblock ~6 ~ ~4 minecraft:wheat[age=7]
function spm_vr:_lib/spawn_ally
summon playermob:player_mob ~3 ~1 ~6 {Tags:["spm_vr.mob","spm_vr.contender"],PersistenceRequired:1b}
...
tellraw @a [{"text":"[spm_vr] crop_multi_mob (VR-T3k) — two allies; one crop must not double-harvest","color":"gold"}]
