...
| Scope boundary | Host Creative-only gate, 24-block range, focus-selected lines, text, and Goal authority remain host-owned | Scavenger changes AI or invents objective text while repairing contrast | Host capture call-path `CODE_CONFIRMED`; shader-overlay depth/occluder parity `UNVERIFIED` |
...
| Iris + Photon backend | Captured SPM lines are projected and drawn after world post-processing | Photon directionally darkens the label according to camera/sky direction | `ShaderReadoutOverlayTest` projection coverage + Photon/Iris log and source inspection `CODE_CONFIRMED`; runtime visual `UNVERIFIED` |
| Shadow pass | Iris shadow rendering neither captures nor emits an overlay line | Duplicate/incorrect coordinates from the shadow camera | `isRenderingShadowPass` guard `CODE_CONFIRMED`; runtime `UNVERIFIED` |
