...
| exact v0.8.0 artifact | reflective linkage plus autonomous high-value SELL→block→ordinary emerald BUY | fake player/menu, board insertion, price reconstruction, marker loss | **`RUNTIME_CONFIRMED / W2 PASS`**: `1 enchanted stick -> 10 blocks`; exact Q2 identity; `10 blocks -> staged 8 blocks + 18 emeralds -> final 8 blocks + 8 emeralds + iron pickaxe` |
| accepted witness removal | fixture/tracker/commands, passive hooks, lifecycle wiring, tags, and temporary tests are absent from production source/JAR | temporary diagnostics silently ship or production TE policy/source/normalization is removed | **W2.4 PASS** — 1614 tests/0 failures; 0 production-source refs; 0 witness JAR entries; all 5 project-owned TE compat classes retained |

Runtime verification: exact Trade Everything 0.8.0 W2 positive path **PASS**, including physical
block payout, exact Q2 identity, staged-only minimum denomination conversion, exact debit/change,
single sale/notify, and iron-pickaxe acquisition. W2.4 clean production artifact:
`build/libs/spmscavenger-1.11.0.jar`, SHA-256
`5EF3639FF03DA20191C3C83BCF662461DB081A8ABFA00E2CEBDC8C93A8B49BF9`; **1614 tests, zero
failures/errors/skips; zero temporary witness entries; zero packaged upstream TE entries**.
