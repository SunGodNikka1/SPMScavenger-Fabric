...
| Iris + Photon backend | Exactly one captured SPM line is projected and drawn after world post-processing at the mob billboard's screen position | Photon directionally darkens the label, the dark host pass remains, or the HUD copy appears detached at the left edge | Empty-string host suppression + Fabric world-context projection snapshot + `ShaderReadoutOverlayTest` transform-order coverage `CODE_CONFIRMED`; prior duplicate screenshot `RUNTIME_CONFIRMED FAILURE`; rebuilt visual `UNVERIFIED` |
